...
const updateIban = function(value){
	let iban = generateIban(value);
	$("#iban").html(iban);
}

const generateIban = function(ccc){
	ccc = ccc.split(" ").join("");
	let iban = ccc;
	let countryCode = "ES"
	let countryAsNumber = "142800"
	let controlDigits = "";

	let auxIban = ccc + countryAsNumber;
...
const updateBic = function(value){
	ccc = value.split(" ");
	bankCode = ccc[0];
	let bic = getBic(bankCode);
	if(bic == undefined)
		bic = "<span class='font-weight-bold text-danger'>LA CUENTA ES ERRONEA O NO EXISTE</span>";
	$("#bic").html(bic);
}
